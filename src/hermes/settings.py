...
def get_config_folder() -> str:
    ...
    Returns:
        str: The value of HERMES_CONFIG_FOLDER environment variable
    """
    hermes_config_folder = os.getenv('HERMES_CONFIG_FOLDER')
    if not hermes_config_folder:
        raise ValueError(
            'HERMES_CONFIG_FOLDER environment variable must be set'
        )
    else:
        return hermes_config_folder
...
def get_artifacts_folder() -> str:
    ...
    if not hermes_artifacts_folder:
        raise ValueError(
            'HERMES_ARTIFACTS_FOLDER environment variable must be set'
        )
    else:
...
def get_definition_file_path() -> pathlib.Path:
    ...
    definition_file_path = pathlib.Path(
        artifact_folder, HERMES_DEFINITIONS_FILE
    )
